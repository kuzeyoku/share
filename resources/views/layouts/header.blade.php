<div id='HeaderZone' ismodulecontainer='true'
    class='HeaderContainer HeaderZoneContainer ZoneContainer HeaderZone ModuleContainer clearfix '>
    <div class="ModuleItem  StaticModule  mhidden-xs" wo="1678" id="module_502260665">
        <div class="ModuleGridGiant layout-101 layout-color- module_502260665 clearfix">
            <div class='BodyCenter BodyCenter502260665 clearfix'>
                <div class='ModuleGridContainer  ModuleGridContainer502260665' gridswidthmode='1'>
                    <div class='row ModuleSubContainer'>
                        <div id='Sub502260665_1'
                            class='ModuleContainer SubContainer ModuleGridItem     col-xs-12 col-sm-12 col-md-12 col-lg-12'
                            positiontype='2' ismodulecontainer='true'>
                            <div class="ModuleItem  StaticModule  " wo="1678" id="module_502260672">
                                <div class="ModuleCustomCodeGiant layout-101 layout-color- module_502260672 clearfix">
                                    <div class='BodyCenter BodyCenter502260672 clearfix'>
                                        <div class="nav-main" style="display: flex; justify-content: space-between;">
                                            <div class="nav-left">
                                                <div style="width: 4vw"></div>
                                                <section class="nav-logo">
                                                    <a class="nav-link" href="/ ">
                                                        <img class="nav-img"
                                                            src="https://cdn-cloudflare.meidianbang.cn/comdata/81833/202308/202308011456390e5da6.png"
                                                            style="width: 130px; background: none;">
                                                    </a>
                                                </section>
                                                <div style="width: 4vw"></div>
                                                <section class="nav-content">
                                                    <div class="menu">
                                                        <div class="nav-item">
                                                            <span class="nav-title">Product</span>
                                                        </div>
                                                        <div class="product-row">

                                                            <div class="product-item">
                                                                <div class="product">
                                                                    <div>
                                                                        <img class="product-img"
                                                                            src="https://cdn-cloudflare.meidianbang.cn/comdata/99450/202306/202306201119298a0443.png"
                                                                            style="background: none;">
                                                                    </div>
                                                                    <div class="product-text">
                                                                        <a class="product-title" name="a4503">Classic
                                                                            Series</a>
                                                                        <a href="/102PV2" class="product-label">SHARE
                                                                            102S PRO V2</a>
                                                                        <a href="/202s" class="product-label">SHARE
                                                                            202S PRO V2</a>
                                                                        <a href="/303V2" class="product-label">SHARE
                                                                            303S PRO V2</a>
                                                                    </div>
                                                                </div>
                                                                <div class="product">
                                                                    <div>
                                                                        <img class="product-img"
                                                                            src="https://cdn-cloudflare.meidianbang.cn/comdata/99450/202306/20230620112121335586.png"
                                                                            style="background: none;">
                                                                    </div>
                                                                    <div class="product-text">
                                                                        <a class="product-title" name="a3008">Gimbal
                                                                            Series</a>
                                                                        <a href="/T2" class="product-label">SHARE
                                                                            T2</a>
                                                                        <a href="/V3S" class="product-label">PSDK
                                                                            102S
                                                                            V3</a>
                                                                        <a href="/203" class="product-label">SHARE
                                                                            203S PRO</a>
                                                                        <a href="/304" class="product-label">SHARE
                                                                            304S PRO</a>
                                                                        <a href="/100m" class="product-label">SHARE
                                                                            100M PRO</a>
                                                                        <a href="/6100x" class="product-label">SHARE
                                                                            6100 SDK</a>
                                                                        <a href="/2600" class="product-label">SHARE
                                                                            2600 SDK</a>
                                                                    </div>
                                                                </div>


                                                                <div class="product">
                                                                    <div>
                                                                        <img class="product-img"
                                                                            src="https://cdn-cloudflare.meidianbang.cn/comdata/81833/202407/20240711172158fb6971.png"
                                                                            style="background: none;">
                                                                    </div>
                                                                    <div class="product-text">
                                                                        <a class="product-title" name="a4516">Lidar
                                                                            Scanner</a>
                                                                        <a href="/S10" class="product-label">SHARE
                                                                            SLAM S10</a>
                                                                        <a href="/capture" class="product-label">SHARE
                                                                            Capture</a>
                                                                    </div>
                                                                </div>
                                                                <div class="product">
                                                                    <div>
                                                                        <img class="product-img"
                                                                            src="https://cdn-cloudflare.meidianbang.cn/comdata/99450/202306/20230620111929e2e802.png"
                                                                            style="background: none;">
                                                                    </div>
                                                                    <div class="product-text">
                                                                        <a class="product-title"
                                                                            name="a4516">Ecosysytem</a>
                                                                        <a href="/MOZU" class="product-label">SHARE
                                                                            Image Module </a>

                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="menu">
                                                        <div class="nav-item">
                                                            <span class="nav-title">Support</span>
                                                        </div>
                                                        <div class="tip-row">
                                                            <div class="tip">
                                                                <a href="/Content/485514">
                                                                    <div class="tip-item">
                                                                        <div>
                                                                            <img class="product-img"
                                                                                src="https://cdn-cloudflare.meidianbang.cn/comdata/99450/202306/2023062011394031d908.jpg"
                                                                                style="background: none;">
                                                                        </div>
                                                                        <div class="tip-text">
                                                                            <div class="product-title">
                                                                                Support</div>
                                                                            <div class="product-label">
                                                                                Download</div>
                                                                        </div>
                                                                    </div>
                                                                </a>
                                                                <a href="/Content/1901561"
                                                                    style="margin-left: -70px;">
                                                                    <div class="tip-item">
                                                                        <div>
                                                                            <img class="product-img"
                                                                                src="https://cdn-cloudflare.meidianbang.cn/comdata/81833/202308/20230801104916daa2c2.jpg"
                                                                                style="background: none;">
                                                                        </div>
                                                                        <div class="tip-text">
                                                                            <div class="product-title">
                                                                                SHARE Data Manager</div>
                                                                            <div class="product-label">
                                                                                Post-processing Software
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </a>
                                                                <a href="http://o.shareuavtec.com:5555/"
                                                                    style="margin-left: -20px;">
                                                                    <div class="tip-item">
                                                                        <div>
                                                                            <img class="product-img"
                                                                                src="https://cdn-cloudflare.meidianbang.cn/comdata/81833/202308/202308011055058572f6.jpg"
                                                                                style="background: none;">
                                                                        </div>
                                                                        <div class="tip-text">
                                                                            <div class="product-title">
                                                                                SHARE CALCULATION</div>
                                                                            <div class="product-label">
                                                                                Aerial Survey Parameter
                                                                                Calculator</div>
                                                                        </div>
                                                                    </div>
                                                                </a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="menu">
                                                        <a href="/MOD" class="nav-item nav-link"><span
                                                                class="nav-title">Online Model</span>
                                                        </a>
                                                    </div>

                                                    <div class="menu">
                                                        <a href="/Content/484207" class="nav-item nav-link"><span
                                                                class="nav-title">About us</span>
                                                        </a>
                                                    </div>
                                                </section>
                                            </div>
                                            <div class="nav-tool">
                                                <div class="nav-btn">
                                                    <a href="/CONTACTUS" class="nav-link">
                                                        <div class="nav-buttonbox">
                                                            <span>Request A Demo</span>
                                                        </div>
                                                    </a>
                                                </div>
                                                <div class="nav-lang">
                                                    <a href="https://www.shareuav.cn/" class="nav-link">
                                                        <img class="nav-img"
                                                            src="https://cdn-cloudflare.meidianbang.cn/comdata/81833/202202/20220209173556cba9c0.png"
                                                            style="background: none; padding-left: 20px;">
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class='clearfix visible-lg'></div>
                        <div class='clearfix visible-xs'></div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
